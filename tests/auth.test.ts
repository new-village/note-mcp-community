...
async function tempConfigPath() {
  const dir = await mkdtemp(join(tmpdir(), 'note-mcp-test-'));
  tempDirs.push(dir);
...
describe('note auth storage', () => {
  it('prefers NOTE_COOKIE over stored config cookie', async () => {
    ...
    await expect(readCookie({ env: { NOTE_COOKIE: 'env=1' }, configPath })).resolves.toBe('env=1');
  });
  ...
  it('saves config file without leaking cookie in status', async () => {
    ...
    await expect(readFile(configPath, 'utf8')).resolves.toContain('session=secret-cookie-value');
