...
describe("cookiesToHeader", () => {
  ...
  it("throws when no note.com cookie exists", () => {
    expect(() =>
      cookiesToHeader([{ name: "other", value: "def", domain: "example.com" }]),
    ).toThrow(/No note\.com cookies/);
  });
});
  ...
  it("includes the config file path when the cookie is saved", () => {
    expect(
      buildBrowserLoginResult("fp=browser-cookie-value", true, {
        configPath: "/Users/kazu/.config/note-mcp-community/config.json",
      ...
      cookiePreview: "fp=b…alue",
      message:
        "note.com authentication configured from browser login. Cookie saved to /Users/kazu/.config/note-mcp-community/config.json.",
    });
  });
});
